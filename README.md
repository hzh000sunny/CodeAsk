...
- v1.0.4 已新增独立 `src/codeask/agent/opencode_compat/` 模块，默认会话接入 shared `opencode serve`、会话级 workspace、Wiki 零复制挂载、remote MCP、opencode 事件流、显式 OpenCode Provider 选择/手动测试和真实浏览器 live E2E 通道。LLM 配置编辑态连接测试遵循表单语义：先测试当前草稿，保存时才把测试状态写入数据库。
