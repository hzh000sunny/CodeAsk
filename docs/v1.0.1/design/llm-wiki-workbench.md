...
| `POST` | `/api/wiki/imports` | 创建导入任务并保存 staging 文件；若 preflight 有冲突则阻断 |
...
- preflight 输入为 `multipart files[]`，每个上传文件的 `filename` 字段携带相对路径。
- 手工上传单资源与目录批量导入的差异。
- 成功导入文件数量。
- 跳过文件数量。
- 冲突路径。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| `GET` | `/api/wiki/search?q=&feature_id=` | Wiki 搜索第一版；当前覆盖正式 Wiki 文档和问题报告 |
...
- 搜索第一版当前返回 `group_key/group_label` 供前端直接渲染，后续再扩展到历史特性和 Agent 场景。
...
- 阅读态默认不展示导入任务状态栏，也不展示常驻元信息条；来源、更新时间、索引状态、断链、历史版本都通过抽屉或轻量浮层进入。
...
- 已落地 Wiki 搜索第一版，当前支持按当前特性文档和问题报告分组展示结果。
- 已落地编辑态离开确认和目录树节点三点菜单，支持新建目录、新建 Wiki、重命名、删除。
- 当前详情抽屉仍未完整展示路径和更新时间；历史特性树和更完整的搜索分桶仍待补。
