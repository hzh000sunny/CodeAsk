...
- 当前已实现 `GET/POST/PUT/DELETE /api/wiki/nodes*` 的最小读写能力；v1.0.1 第一版对普通 Wiki 写操作采用“全员可写”覆写，同时继续保留系统目录保护，以及未来 `owner / admin` 权限收敛通道。
...
- 当前已补上独立 `wiki/index` 和 `wiki/maintenance` 第一版边界：`WikiIndexService` 负责刷新 native wiki document 的派生状态，`POST /api/wiki/maintenance/nodes/{node_id}/reindex` 负责管理员治理向的手动 repair subtree。
- 当前 `publish / rollback / import apply` 已通过 `WikiIndexService` 收口正式文档状态刷新，不再把这部分逻辑继续散落到多个 service 内。
- 当前已补上 Wiki 收尾治理入口：来源治理抽屉、软删除节点恢复、历史特性恢复、手动 reindex，以及会话附件晋级为正式 Wiki 的前后端编排。
...
- 编辑态源码和预览同屏，目录树默认收起。
- 节点菜单不被滚动容器裁切。
- v1.0.1 当前版本默认显示普通 Wiki 写入口；管理员治理动作保留权限校验，直接调用失败后有消息提示。
- 自动草稿在刷新后可恢复。
- 版本历史可查看、对比、回滚。
- 导入冲突列表可读，不自动覆盖。
