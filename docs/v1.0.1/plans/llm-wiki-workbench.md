...
以下能力仍属于 v1.0.1 的尾项，当前版本还不能宣称“全部完成”：

- 来源注册表的完整前端管理 UI：需要把 `wiki_sources` 的列表、编辑、同步结果和 provenance 明细真正暴露给用户，而不只是后端接口可用。
- 会话附件晋级、软删除恢复、历史特性恢复、手动 reindex repair 的前端入口：当前后端链路已存在，但 Wiki 工作台里还没有完整管理面板。
- Agent 侧 Wiki 范围解析的进一步增强：当前已支持多特性范围透传和可点击证据，但更复杂的口语化目录解析、跨目录组合和报告细粒度锚点仍需继续收口。
- 真实浏览器的回归矩阵仍需继续补齐：尤其是目录树拖拽排序、恢复通道和来源治理相关操作。
