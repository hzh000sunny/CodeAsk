...
> 版本：v1.0.5
> 状态：**已延后为后续 repo 里程碑底稿**。2026-05-31 架构复核曾推翻"删除残留"冻结结论并确认 M11-B 技术方向可行；2026-06-01 负责人决策把“代码仓内容进入 OpenViking”延后，当前 v1.0.5 release 不实现 repo 内容同步。开发第二轮调研结论见 §0–§5（保留原始证据），架构复核更正与最终方向见 **§6**。
...
>
> ⚠️ **读这份文档先看 §6**：§0–§5 是开发第二轮的结论（曾据此冻结 M11-B），其核心阻塞"删除后 find 仍返回已删 URI"在 §6 的架构复核重测中被推翻——删除本身干净，该现象经受控复现复不出、疑似异步竞态而非确定性 bug。M11-B 因此曾经**解冻、方向已定**；但 repo 内容同步现已延后，不进入 v1.0.5 RC 范围。
