...
> **给 agentic worker：** 实施本计划时必须使用 `superpowers:subagent-driven-development`（推荐）或 `superpowers:executing-plans`，按任务逐项执行。所有步骤使用 checkbox（`- [ ]`）追踪。涉及管理员 UI 的改动必须先触发 `frontend-design`（落地实现）或 `ui-ux-pro-max`（评审/方向）skill，严格沿用现有 OpenViking dashboard / Settings 工作台风格，不要堆叠通用 AI 审美。
...
- [ ] 在 `src/codeask/agent/opencode_compat/` 新增 `permissions.py`（纯函数 + dataclass，无 IO）：
  - [ ] `@dataclass(frozen=True) OpencodeToolPermissions`：`tools: dict[str,str]`、`bash_mode: Literal["allow","deny","whitelist"]`、`bash_patterns: tuple[str,...]`。
  - [ ] `DEFAULT_TOOL_PERMISSIONS`：与现有 `READONLY_PERMISSION`（不含 bash）一致 + OV 写工具 deny + `webfetch:"deny"`。给出 `default()` classmethod 返回与今日行为等价的实例（bash_mode="deny"）。
  - [ ] `GOVERNED_TOOLS`（顺序固定，供 UI 目录）与 `OPENVIKING_WRITE_TOOLS` 子集常量。
  - [ ] `BASH_WHITELIST_SUGGESTIONS`：推荐只读命令模板元组。
  - [ ] `from_stored(value: object) -> OpencodeToolPermissions`：宽松解析 DB JSON，未知键忽略、缺失键回落默认、非法 mode/值回落默认；保证永不抛错（DB 脏数据不能拖垮会话创建）。
  - [ ] `to_stored(self) -> dict`：规范化序列化（用于 PATCH 落库）。
  - [ ] `to_permission_block(self, *, openviking_enabled: bool) -> dict[str,object]`：产出 opencode `permission` 子块（不含 `external_directory`）。bash：`allow`/`deny` → 字符串；`whitelist` → `{"*":"deny", **{p:"allow" for p in patterns}}`（空 patterns 时退化为 `"deny"` 并记一条 warning 级日志）。OV 未启用时**剔除** `openviking_*` 键（与现状一致：只有 OV 启用才注入这些 deny）。
- [ ] 校验函数 `validate_bash_patterns(patterns) -> list[str]`：去空白、去重、保序；单条长度上限（如 ≤200）、总条数上限（如 ≤64）；拒绝含换行/控制字符的条目。超限抛 `ValueError`（API 转 400）。

## 2. 改造权限构建（config.py）

- [ ] `_build_permission()` 增加可选参数 `tool_permissions: OpencodeToolPermissions | None = None`：
...
- [ ] `build_opencode_config()` 与 `OpenCodeConfigInput` 增加 `tool_permissions: OpencodeToolPermissions | None = None` 字段；`with_openviking` 与 backend 的 `_with_profile`（backend.py:772）透传该字段（注意 `_with_profile` 是手写逐字段拷贝，**必须补这一字段**，否则会丢失）。
- [ ] 保持 `READONLY_PERMISSION` / `OPENVIKING_WRITE_TOOL_DENIES` 常量不删（默认路径仍用），避免大范围连锁改动。

## 3. resolver 注入（backend.py + app.py）

- [ ] `OpenCodeCompat.__init__` 增加 `tool_permissions_resolver: Callable[[], Awaitable[OpencodeToolPermissions]] | None = None`（无 session 入参——权限是全局配置，非每会话）。
- [ ] 新增 `async def _resolve_tool_permissions(self) -> OpencodeToolPermissions`：resolver 为 None 时返回 `OpencodeToolPermissions.default()`；否则 await resolver，异常时 log + 回落 default（绝不阻断会话创建）。
- [ ] `initialize_session()`：在构造 `config_input` 后、`build_opencode_config` 前解析一次，传入 `tool_permissions`。确认 `_config_input` / `_with_profile` 链路把该字段带到 `build_opencode_config`。
- [ ] [app.py](/home/hzh/workspace/CodeAsk/src/codeask/app.py) 在构造 `OpenCodeCompat`（app.py:221）处注入 resolver：`async def resolve_tool_permissions() -> OpencodeToolPermissions: return await load_opencode_tool_permissions(factory)`，其中 loader 读 `system_settings` 的 `opencode_tool_permissions` 键并 `OpencodeToolPermissions.from_stored(...)`。
- [ ] loader 放在新 API 模块或一个小的 `opencode_compat/permissions_store.py`（接受 `session_factory`），与 openviking 的 `_resolve_openviking_mcp_config` 风格一致。
...
- [ ] `GET /admin/opencode/permissions` → 返回当前配置 + 目录元数据：
  ```json
  {
...
- [ ] `PUT /admin/opencode/permissions`（整体覆盖式更新，payload 经 Pydantic + `validate_bash_patterns` 校验）：
  ...
  - 返回与 GET 同形的最新值。
  - 校验失败 → 400（含可读信息）。
- [ ] 在 [app.py:480](/home/hzh/workspace/CodeAsk/src/codeask/app.py) 附近 `include_router(opencode_admin_router, prefix="/api")`。
- [ ] Pydantic 模型：`mode` 用 `Literal`；tool 值用 `Literal["allow","deny"]`；只接受治理集内的 key（未知 key 拒绝或忽略，二选一并写测试固定行为）。

## 5. 前端类型与 API 客户端

- [ ] `frontend/src/types/api.ts`：新增 `OpencodeToolPermissionValue = "allow"|"deny"`、`OpencodeBashMode`、`OpencodePermissionsResponse`（含 `catalog`/`defaults`/`openviking_enabled`）、`OpencodePermissionsUpdateRequest`。
- [ ] 新增 `frontend/src/lib/api-opencode.ts`（或并入既有 api 模块）：`getOpencodePermissions()`、`updateOpencodePermissions(payload)`。复用现有 fetch 封装与错误处理。
...
### 6.1 导航与页面装配
- [ ] [SettingsPage.tsx](/home/hzh/workspace/CodeAsk/frontend/src/components/settings/SettingsPage.tsx)：
  - [ ] 把 `runtime` 项 `label` 改为 `"OpenCode"`，`description` 改为如「opencode 后端状态与工具权限」，图标可保留 `Activity` 或换 `Terminal`。
  - [ ] 在 `adminSettingsPages` 数组里把该项移动到 `openviking` 项**正上方**（保持其余顺序）。
  - [ ] 同步更新 `pageDescriptions["runtime"]`。
  - [ ] （决策点）是否把路由 id `"runtime"` 改名为 `"opencode"`：**推荐保留 id 不变**（避免改 routing.ts 联合类型/白名单与潜在深链回归），仅改展示 label。若改名，则同步 [routing.ts](/home/hzh/workspace/CodeAsk/frontend/src/lib/wiki/routing.ts) 的 `SettingsAdminPageId` 与 `readSettingsAdminPage`。
- [ ] `AdminRuntimeSettings`（GlobalSettings.tsx:92）：在 `OpencodeStatusPanel` 下追加新组件 `<OpencodeToolPermissionsPanel />`。

### 6.2 工具权限面板（新组件）
- [ ] 新建 `frontend/src/components/settings/OpencodeToolPermissionsPanel.tsx`：
  - [ ] `useQuery(["admin-opencode-permissions"], getOpencodePermissions)` 拉取；本地表单 state，`useMutation` 提交（`isPending` 态、成功/失败 toast 走 `AppFeedback`）。
  - [ ] 每个治理工具一行：工具名 + 简述 + allow/deny 控件（`SwitchControl` 或分段按钮，二态）。OV 写工具仅当 `openviking_enabled` 渲染，并带风险提示样式（danger 调性）。
  - [ ] `bash` 行特殊：三态分段控件（允许 / 拒绝 / 白名单）。选「白名单」时展开 pattern 编辑区——chips 或可增删的输入行；提供「填入推荐命令」按钮（来自 `catalog.bash_suggestions`）。空白名单给出内联提示「等价于拒绝」。
  - [ ] 「保存」`.button-primary`：`requestConfirm` 二次确认（说明“对新建会话生效，不影响进行中的会话”）。「恢复默认」`.button-secondary`：用 `defaults` 重置表单（仅本地，保存后才落库）。
  - [ ] 表单 dirty 检测：未改动时禁用保存。
- [ ] `frontend/src/styles/globals.css`：新增本面板所需类，复用既有 token（边框 #e4e9f2/#d0d5dd、文字 ramp、success/error 配色、7–8px 圆角、36px 控件、字重 650–750）。保持栅格对齐，避免之前 OpenViking 字段提示破坏对齐的同类问题。

## 7. 测试

### 7.1 后端单元（pytest）
- [ ] `tests/unit/test_opencode_permissions.py`：
  - [ ] `default()` 产出与现有 `READONLY_PERMISSION`(+OV deny) 等价的 permission block（OV 启用/未启用两种）。
  - [ ] `from_stored` 宽松解析：缺键回落、未知键忽略、非法 mode/值回落、`None`/非 dict 输入返回 default。
  - [ ] `to_permission_block`：bash allow/deny/whitelist 三态正确；whitelist 空 patterns 退化为 deny；OV 未启用时不含 `openviking_*` 键。
  - [ ] `validate_bash_patterns`：去重保序、超长/超量/含控制字符抛错。
- [ ] `tests/unit/test_opencode_compat_backend.py` / config 测试：`build_opencode_config` 传入自定义 `tool_permissions` 时 `permission` 块正确，且 `external_directory` 仍叠加；`_with_profile` 透传不丢字段。

### 7.2 后端集成（pytest，httpx ASGI）
- [ ] `tests/integration/test_opencode_admin_api.py`：
  - [ ] GET 默认返回（无 DB 配置）= defaults，且含 catalog/openviking_enabled。
  - [ ] PUT 合法 payload → 落库 + 审计 + 回读一致。
  - [ ] PUT bash whitelist → 回读 patterns 规范化。
  - [ ] PUT 非法（超量 patterns / 非法 mode / 越权工具 key）→ 400。
  - [ ] 非管理员 → 403。
- [ ] resolver 端到端：构造带 resolver 的 `OpenCodeCompat`（或直接测 loader），DB 写入配置后 `initialize_session` 产出的 `opencode.json` permission 反映配置（可在现有 backend 集成测试夹具上加用例）。

### 7.3 前端（Vitest）
- [ ] `frontend/tests/opencode-permissions.test.tsx`：渲染面板、mock GET、切换工具态与 bash 三态、白名单增删与「填入推荐」、保存调用 `updateOpencodePermissions` 且 payload 正确、OV 未启用时不渲染 OV 写工具行、dirty 禁用逻辑。
- [ ] `frontend/tests/settings-page.test.tsx`：导航项 label=「OpenCode」、位于 OpenViking 之上；进入该页渲染状态面板 + 权限面板。

### 7.4 E2E（Playwright，按现有约定）
- [ ] 管理员进入 OpenCode 页 → 把 bash 设为白名单并填入 `git *`/`ls *` → 保存成功 toast → 回读保持。

## 8. 文档与验收

- [ ] 更新 [acceptance-checklist.md](/home/hzh/workspace/CodeAsk/docs/v1.0.5/plans/acceptance-checklist.md)：新增 M14 验收条目（导航重命名+排序、每工具 allow/deny 可配、bash 白名单生效于新会话、默认行为不变、非管理员 403、生效语义提示）。
- [ ] 自测记录：`uv run pytest`（新单测+集成）、`ruff`/`pyright` clean；前端 `lint`/`build`/相关 vitest 绿；说明“对进行中会话不追溯”这一已知语义。
