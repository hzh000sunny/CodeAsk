...
`GET /api/sessions/{id}/turns` 用于恢复历史会话消息。后端必须先按当前 `subject_id` 加载会话，只允许会话所有者读取；响应按 `turn_index, created_at` 排序返回 `SessionTurnResponse[]`，字段包含 `id`、`session_id`、`turn_index`、`role = user / agent`、`content`、`evidence`、`created_at`、`updated_at`。前端刷新页面或切换会话时必须读取该接口渲染聊天流，不能只依赖本地 React 状态或 SSE 直播期间的临时消息。

`GET /api/sessions/{id}/traces` 用于恢复历史调查进度和运行事件。后端必须先按当前 `subject_id` 加载会话，只允许会话所有者读取；响应返回 `AgentTraceResponse[]`，字段包含 `id`、`session_id`、`turn_id`、`stage`、`event_type`、`payload`、`created_at`、`updated_at`。接口会过滤高噪声 LLM delta，仅保留阶段事件、决策事件、工具调用结果和必要的 LLM `message_start / tool_call_done / error` 事件。前端刷新页面或切换会话时必须读取该接口回放右侧“调查进度”和“运行事件”，并把工具 payload 摘要化展示，避免直接把大段 JSON 暴露给用户。
...
`/api/repos` 采用同一路径分角色控制：`GET /api/repos` 供会话和特性页面读取全局仓库池；`POST /api/repos`、`PATCH /api/repos/{id}`、`DELETE /api/repos/{id}` 和 `POST /api/repos/{id}/refresh` 仅管理员可调用。当前实现没有 `/api/admin/repos` 路径。

仓库编辑语义：

- `PATCH /api/repos/{id}` 可修改 `name`、`source`、`url`、`local_path`。
- 只修改 `name` 不改变 `status`，也不投递同步任务。
- 修改 `source` / `url` / `local_path` 会清空不再适用的另一类位置字段，清除 `error_message`，并重新投递 clone / sync 任务。
- `POST /api/repos/{id}/refresh` 在 UI 中显示为“同步”或“重试同步”，不是普通页面刷新。`ready` 仓库会在不删除 bare 缓存目录的前提下执行 fetch 更新；`failed` / `registered` 仓库用于重试。应用启动后还会按小时调度一次全量仓库同步，跳过 `cloning` 状态，避免与正在进行的 clone 冲突。

`/api/skills` 当前承载“分析策略”而不是完整 Skill Package。路径保持不变是为了兼容现有数据表和前端调用；产品文案使用“分析策略”。全局策略写操作要求管理员身份，特性策略写操作跟随特性管理权限。响应字段包含 `id`、`name`、`scope`、`feature_id`、`stage`、`enabled`、`priority`、`prompt_template`。
...
0017 analysis policy fields on skills
```
