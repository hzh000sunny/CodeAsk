...
**一期普通用户仍然无鉴权，但增加内置管理员登录保护全局配置。**

| 项 | 一期决策 |
|---|---|
| 普通用户登录 | 无，直接访问 URL 使用 |
| 管理员登录 | 内置 admin 登录，保护全局配置和仓库管理 |
| 鉴权 | 普通使用依旧依赖私有部署边界；管理员功能依赖 HttpOnly admin cookie |
| 身份 | "自报身份"软识别（PRD §4.2 / §4.4.1）：首次访问生成 `client_id` 存 localStorage；用户菜单可选填昵称；`subject_id = nickname@client_id`（无昵称时为 `device@client_id`） |
| 读写权限 | 普通用户可管理自己的会话、用户设置、用户 LLM 配置；管理员额外管理全局配置和仓库池 |
| 会话归属 | 会话 / 报告 / 文档等写操作记录 `subject_id`；UI 默认按 `subject_id` 只显示当前用户会话 |

**与旧版差异**：旧设计有"单一 master token"鉴权，一期不恢复单 token。管理员登录只用于全局配置隔离，不替代未来 AuthProvider。原因：
...
管理员 cookie 使用 HttpOnly，前端不能读取 cookie 内容，只通过 `/api/auth/me` 获取当前角色。
...
| slot | 默认（一期） | 用于 |
|---|---|---|
| `<LoginPage />` | 中性登录页；认证成功后可能获得内置管理员身份（默认账号 `admin` / `admin`） | 登录入口 |
| `<UserMenu />` | 显示匿名用户或内置管理员，提供管理员登录 / 退出入口 | 顶栏用户菜单 |
| `<UserProfilePage />` | 空 | 用户资料页 |
| `<RoleGate role="admin" />` | 仅内置管理员通过；普通用户看不到全局配置入口 | 包裹需要管理员权限的操作 |

### 4.3 权限模型（未来）

| 阶段 | 权限模型 |
|---|---|
| 一期（MVP） | 普通用户匿名使用，内置 `admin` 保护全局 LLM 配置和仓库管理 |
| MVP+ | 两级增强：`member`（读 + 提问 + 验证报告）/ `admin`（删除特性 / 删除仓库 / 系统配置） |
