...
>
> 跨版本稳定规则见 `../../rules/problem-report.md`。本文关注 v1.0 设计；当局部实现与跨版本报告规则冲突时，应以后者为准。
...
Markdown 正文的跨版本规则见 `../../rules/problem-report.md`：

- 标题必须为 `YYYY-MM-DD 问题描述`
- `问题描述` 由 AI 先行生成，但允许用户修改
- 正文由 AI 自主组织，不强制固定章节名
- 证据不足时允许生成草稿，但必须显式表达未确认项和待补充项

本设计中的章节示例只作为信息覆盖参考，不是固定输出模板。
...
- 用户问题
- Agent 的报告正文初稿（不是聊天记录复写，而是按报告目标成文）
- AI 生成的默认标题（最终格式仍必须满足 `YYYY-MM-DD 问题描述`）
