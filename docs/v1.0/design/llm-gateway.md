...
当前运行时在未显式指定 `config_id` 时，还会执行第一版 LLM 配置选择策略：

- 显式 `config_id` 请求不参与全局池随机选择、会话限额、会话粘性和失败切换。
- `subject_id` 有启用的用户配置时，优先使用用户自己的配置。
- 用户个人配置失败时不自动 fallback 到全局配置。
- 用户没有个人配置时，从启用的全局配置池中随机选择。
- 单个全局配置最近 60 秒最多服务 3 个不同会话。
- 同一 `session_id` 在 60 秒窗口内多次调用同一全局配置，只计为 1 个会话。
- 同一 `session_id` 在 5 分钟内继续使用上次选中的全局配置，避免同一会话短时间内频繁切换模型。
- 如果会话上次使用的全局配置已被删除、禁用或进入失败冷却，直接切换下一个可用配置。
- 全局配置 5 分钟内最终失败 3 次后，临时剔除 10 分钟。
- 全局配置在一次请求初始阶段失败且尚未输出内容时，立即排除该配置并切换下一个可用全局配置；已开始输出后不跨模型续流。
- 非供应商健康类错误不计入失败冷却，例如上下文超限、`max_tokens` 非法、工具 schema 或请求格式错误。
- 全局池无可用配置时返回 `当前资源繁忙，请稍后再试`。

该策略的状态第一版保存在后端进程内存中，不作为数据库持久模型。
