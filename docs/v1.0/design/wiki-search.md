...
> v1.0.2 起，默认会话中的 Wiki / 报告检索只作为候选上下文注入模型，不再由后端检索层生成“范围判断”“充分性判断”或“下一步代码调查”等流程结论。本文中的固定判定链路属于 v1.0 历史设计。
...
当前实现保留 `skills` 表和 `/api/skills` 路径作为兼容层，但产品语义是“分析策略 / Prompt 策略”，不是完整 Skill Package。分析策略是 Markdown 提示词模板，不具备执行能力；它只向 Agent 上下文注入规则文本。
...
特性分析策略后注入，可以覆盖或细化全局规则。未来完整 Skill Package 可以扩展触发条件、工具声明、示例、版本和测试用例；这些不属于当前 v1.0 工作台交付。
