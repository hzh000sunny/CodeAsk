...
**v1.0.1 后续状态（2026-05-06）：**

- 独立 Wiki 工作台已从规划阶段进入持续实现与验收阶段。
- 已落地内容包括：独立一级 `Wiki` 页面、目录树、阅读态 / 编辑态、导入会话、报告投影、全局搜索、历史特性归档与恢复、会话附件晋级、来源注册表和手动 reindex repair。
- 特性页中的 `知识库` tab 已收敛为轻量入口：仅保留当前特性的目录树和正文预览，不继续承载完整 Wiki 管理。
