...
[L1] 特性切片：选定特性的摘要 + 导航索引 + skill     ← 切特性才变
...
event: thought       data: {"text": "<思考说明>"}     # 仅当模型显式输出
event: stage         data: {"name": "preflight_reports", "status": "hit", "count": 2}
...
特性 (Feature)
├── 关联代码仓（多对多，从全局仓库池勾选）
├── 特性专属 skill（在 skills 表中 scope='feature' 且 feature_id 指向本特性的所有记录）
├── 知识库文档区  ← 用户持续添加：业务文档、技能手册、配置说明…
└── 定位报告区    ← AI 生成 + 人工验证后归档；特性内共享、可跨特性搜索

全局 skill 池        ← 默认始终加载到 Agent；通用排障流程
全局代码仓池        ← 注册即缓存，特性按需勾选

Skill 注入规则：
  Agent 系统提示中先注入所有 global skill（通用规则在前）
  → 选中特性后再追加该特性的 feature skill（特定规则在后，可覆盖通用规则）
  没有"勾选关联"动作；feature skill 上传到该特性即自动生效
```
