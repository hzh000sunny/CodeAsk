...
describe("SettingsPage LLM configuration", () => {
  ...
  it("shows only user settings to ordinary members", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "个人 LLM 配置" })).toBeInTheDocument();
  });
  ...
  it("creates a personal LLM config from user settings", async () => {
    ...
    fireEvent.change(screen.getByLabelText("配置名称"), { target: { value: "个人模型" } });
    const protocolSelect = screen.getByLabelText("消息接口协议");
    expect(protocolSelect).toHaveValue("openai");
    expect(within(protocolSelect).getByRole("option", { name: "OpenAI" })).toHaveValue("openai");
    expect(within(protocolSelect).getByRole("option", { name: "Anthropic" })).toHaveValue("anthropic");
    ...
    fireEvent.change(screen.getByLabelText("Base URL"), { target: { value: "http://llm.internal/v1" } });
    fireEvent.change(screen.getByLabelText("API Key"), { target: { value: "sk-test-abc" } });
    fireEvent.change(screen.getByLabelText("模型名称"), { target: { value: "qwen3-coder" } });
    ...
    const [, init] = fetchMock.mock.calls.find(([path, options]) =>
      path === "/api/me/llm-configs" && (options as RequestInit | undefined)?.method === "POST"
  ...
  it("shows a visible message when creating a LLM config fails", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === "/api/me/llm-configs" && init?.method === "POST") {
        return jsonResponse({ detail: "llm config name already exists" }, 409);
      }
      if (path === "/api/me/llm-configs") {
        return jsonResponse([]);
      }
    ...
    fireEvent.click(screen.getByRole("button", { name: "添加 LLM 配置" }));
    fireEvent.change(screen.getByLabelText("配置名称"), { target: { value: "重复模型" } });
    fireEvent.change(screen.getByLabelText("API Key"), { target: { value: "sk-test-abc" } });
    fireEvent.change(screen.getByLabelText("模型名称"), { target: { value: "qwen3-coder" } });
    fireEvent.click(screen.getByRole("button", { name: "保存 LLM 配置" }));

    expect(await screen.findByRole("alert")).toHaveTextContent("保存 LLM 配置失败");
    expect(screen.getByText(/llm config name already exists/)).toBeInTheDocument();
  });
  ...
  it("edits, toggles, and deletes existing global LLM configs as admin without default controls", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === "/api/admin/llm-configs/llm_2" && init?.method === "PATCH") {
      ...
      if (path === "/api/admin/llm-configs/llm_2" && init?.method === "DELETE") {
    ...
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "设置" }));
    expect(screen.queryByRole("heading", { name: "用户配置" })).not.toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "个人 LLM 配置" })).not.toBeInTheDocument();
    expect(await screen.findByRole("button", { name: "全局配置" })).toBeInTheDocument();
    ...
    expect(screen.getByRole("button", { name: "添加 LLM 配置" })).toHaveClass("button-primary");
    ...
    fireEvent.change(screen.getByLabelText("编辑配置名称"), { target: { value: "主力模型" } });
    fireEvent.change(screen.getByLabelText("编辑消息接口协议"), { target: { value: "anthropic" } });
    fireEvent.change(screen.getByLabelText("编辑 Base URL"), { target: { value: "http://llm.new/v1" } });
    fireEvent.change(screen.getByLabelText("编辑 API Key"), { target: { value: "sk-new-key" } });
    fireEvent.change(screen.getByLabelText("编辑模型名称"), { target: { value: "claude-test" } });
    ...
    const globalSection = screen.getByRole("heading", { name: "全局 LLM 配置" }).closest("section");
    fireEvent.click(within(globalSection as HTMLElement).getByRole("button", { name: "删除" }));
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(
        "/api/admin/llm-configs/llm_2",
